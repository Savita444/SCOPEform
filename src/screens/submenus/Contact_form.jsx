...
const Office = () => {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, "Contact.xlsx"); // Download as Excel file
  };
  useEffect(() => {
    fetchTeam();
  ...
  const fetchTeam = async () => {
    ...
    try {
      const response = await instance.get("contact_form/find", {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm(formData)) {
      ...
      try {
        if (editMode) {
          await instance.put(`contact_form/update/${editingId}`, data, {
            headers: {
              Authorization: "Bearer " + accessToken,
              "Content-Type": "application/json",
            },
          ...
          await instance.post("contact_form/add", data, {
            headers: {
              Authorization: "Bearer " + accessToken,
              "Content-Type": "application/json",
            },
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "20px" }}>
            <button style={{ marginRight: "10px" }}
              className="btn btn-primary" onClick={async () => {
                ...
                const accessToken = localStorage.getItem("accessToken");
                try {
                  await instance.delete(`contact_form/isdelete/${id}`, {
